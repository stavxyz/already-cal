...
limit, never inside a surrogate pair. The rest is kept, with tags stripped
and entities decoded, but its Markdown syntax (such as `**` or
`[text](url)`) stays in the text. The Markdown parser is super-linear on some
inputs, so this bounds the time a hostile description can cost a server.
